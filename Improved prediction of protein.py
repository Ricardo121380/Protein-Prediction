...
class SimpleNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size * seq_len, -1)  # 调整形状为 (batch_size * sequence_length, num_features)
        x = self.fc1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.dropout(x)
        x = self.fc2(x)
        x = x.view(batch_size, seq_len, -1)  # 调整形状回 (batch_size, sequence_length, output_features)
        return x
